...
import { ApiService, AppCurrencyPipe, AuthService, MapPickerComponent, MapLocation, ToastService } from '@shared/public-api';
...
export class ProfileComponent implements OnInit {
  ...
  save() {
    ...
    let completed = 0;
    const finish = () => { if (++completed === 2) { this.saving.set(false); this.toastService.show('Profile saved successfully!', 'success'); } };
    const fail = (err: any) => {
      const e = err.error;
      this.toastService.show(typeof e === 'object' ? Object.values(e).flat().join(' ') : 'Save failed.', 'error');
