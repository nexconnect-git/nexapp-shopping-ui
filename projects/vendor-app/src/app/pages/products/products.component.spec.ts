...
describe('ProductsComponent', () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        provideRouter([
          ...
          { path: 'products/:id/edit', component: BlankComponent },
  ...
  it('creates and loads the product catalogue with health summary data', fakeAsync(() => {
    ...
    expect(
      fixture.nativeElement.querySelector('.low-stock-alert')?.textContent,
    ).toContain('low on stock');
  }));
  ...
  it('debounces search input and sends the query with the first page', fakeAsync(() => {
    ...
    expect(api.getVendorProducts).toHaveBeenCalledWith({
      page: 1,
      search: 'dosa',
    });
  }));
  ...
  it('paginates within bounds and reports nearby page numbers', fakeAsync(() => {
    api.getVendorProducts.and.returnValue(
      of({ results: [product()], count: 100 } as any),
    );
    fixture.detectChanges();
    tick(0);
    api.getVendorProducts.calls.reset();
  ...
  it('classifies product approval and visibility health across edge cases', () => {
    ...
    expect(
      component.needsAttention(
        product({ visibility_blockers: ['Missing photo'] }) as any,
      ),
    ).toBeTrue();
    expect(
      component.needsAttention(
        product({
          visibility_status: 'needs_attention',
          visibility_blockers: [],
        }) as any,
      ),
    ).toBeTrue();
    ...
    expect(
      component.productHealthLabel(
        product({
          visibility_status: 'needs_attention',
          visibility_blockers: undefined,
        }) as any,
      ),
    ).toBe('1 fix needed');
    expect(
      component.productHealthLabel(
        product({ visibility_blockers: ['Missing photo', 'No stock'] }) as any,
      ),
    ).toBe('2 fixes needed');
    expect(
      component.categoryVisibilityLabel(
        product({ category_visibility: 'customer_visible' }) as any,
      ),
    ).toBe('Customer visible');
    expect(
      component.categoryVisibilityLabel(
        product({ category_visibility: 'pending_review' }) as any,
      ),
    ).toBe('Category pending');
    expect(
      component.categoryVisibilityLabel(
        product({ category_visibility: 'hidden' }) as any,
      ),
    ).toBe('No category');
    expect(
      component.approvalClass(product({ approval_status: 'rejected' }) as any),
    ).toContain('rejected');
    expect(
      component.approvalClass(product({ approval_status: undefined }) as any),
    ).toBe('approval-chip draft');
    expect(
      component.approvalText(
        product({
          approval_status: 'pending_approval',
          approval_status_label: undefined,
        }) as any,
      ),
    ...
    expect(
      component.approvalReason(
        product({ approval_status: 'pending_approval' }) as any,
      ),
    ).toBe('Hidden until admin approval.');
    expect(
      component.approvalReason(
        product({
    ...
    expect(
      component.approvalReason(
        product({ approval_status: 'rejected', rejection_reason: '' }) as any,
      ),
    ).toBe('Rejected by admin.');
    expect(
      component.approvalReason(product({ approval_status: 'draft' }) as any),
    ).toBe('Draft products are not customer visible.');
    expect(
      component.approvalReason(product({ is_available: false }) as any),
    ).toBe('Approved but offline.');
    expect(component.canSubmitForApproval(null)).toBeFalse();
    expect(
      component.canSubmitForApproval(
        product({ approval_status: 'approved' }) as any,
      ),
    ).toBeFalse();
    expect(
      component.canSubmitForApproval(
  ...
  it('opens product fixes, submits approval, and updates the targeted row', fakeAsync(() => {
    ...
    api.getVendorProducts.and.returnValue(
      of({ results: [rejected], count: 1 } as any),
    );
    api.submitInheritedProducts.and.returnValue(
      of({ submitted: ['product-1'] } as any),
    );
    ...
    fixture.detectChanges();
    (
      fixture.nativeElement.querySelector('.health-action') as HTMLButtonElement
    ).click();
    fixture.detectChanges();
    expect(
      fixture.nativeElement.querySelector('.fixes-modal')?.textContent,
    ).toContain('Needs better image');

    (
      fixture.nativeElement.querySelector(
        '.fixes-actions .btn-primary',
  ...
  it('shows approval errors from the backend and ignores invalid submissions', fakeAsync(() => {
    ...
    api.submitInheritedProducts.and.returnValue(
      throwError(() => ({ error: { error: 'Already pending' } })),
    );
    ...
    api.submitInheritedProducts.and.returnValue(
      throwError(() => ({ error: { error: { name: ['Required'] } } })),
    );
    component.submitForApproval(validDraft as any);
    tick();
    expect(toast.show).toHaveBeenCalledWith(
  ...
  it('confirms deletion, supports backdrop cancel, deletes through the API, and reloads', fakeAsync(() => {
    ...
    (
      fixture.nativeElement.querySelector('.danger-hover') as HTMLButtonElement
    ).click();
    fixture.detectChanges();
    expect(
      fixture.nativeElement.querySelector('.delete-modal')?.textContent,
    ).toContain('Delete Product?');
  ...
  it('keeps the delete modal open and reports API errors when deletion fails', fakeAsync(() => {
    api.deleteProduct.and.returnValue(
