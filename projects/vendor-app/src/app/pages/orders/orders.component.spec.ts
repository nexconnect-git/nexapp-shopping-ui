...
describe('OrdersComponent', () => {
  ...
  beforeEach(async () => {
    api = jasmine.createSpyObj<ApiService>('ApiService', ['getVendorOrders', 'updateOrderStatus']);
    toast = jasmine.createSpyObj<ToastService>('ToastService', ['show']);
    api.getVendorOrders.and.returnValue(of({ results: [order()], count: 1 } as any));
  ...
  it('shows a loading state while the order request is pending', fakeAsync(() => {
    ...
    expect(fixture.nativeElement.querySelector('.loading-state')?.textContent).toContain('Loading orders');
  ...
  it('renders an empty state for all orders and for filtered views', fakeAsync(() => {
    ...
    expect(fixture.nativeElement.querySelector('.empty-state')?.textContent).toContain('No orders with status "ready"');
  }));
  ...
  it('filters orders by status and resets pagination', fakeAsync(() => {
    ...
    expect(api.getVendorOrders).toHaveBeenCalledWith({ page: 1, status: 'confirmed' });
  }));
  ...
  it('manual reload returns to page one and reloads the current filter', fakeAsync(() => {
    ...
    component.statusFilter = 'preparing';

    fixture.nativeElement.querySelector('.btn-icon').click();
    tick();

    expect(component.page()).toBe(1);
    expect(api.getVendorOrders).toHaveBeenCalledWith({ page: 1, status: 'preparing' });
  }));
  ...
  it('paginates within bounds and exposes nearby page numbers', fakeAsync(() => {
    api.getVendorOrders.and.returnValue(of({ results: [order()], count: 90 } as any));
    fixture.detectChanges();
    tick(0);
    api.getVendorOrders.calls.reset();
  ...
  it('progresses placed, confirmed, and preparing orders from their action buttons', fakeAsync(() => {
    ...
    tick(0);
    fixture.detectChanges();
    (fixture.nativeElement.querySelector('.status-btn.confirm') as HTMLButtonElement).click();
    tick();

    expect(api.updateOrderStatus).toHaveBeenCalledWith('order-1', 'confirmed');
    expect(toast.show).toHaveBeenCalledWith('Order updated to "confirmed".', 'success');
  ...
  it('surfaces status update errors and clears the updating state', fakeAsync(() => {
    fixture.detectChanges();
    tick(0);
    api.updateOrderStatus.and.returnValue(throwError(() => ({ error: { error: 'Kitchen closed' } })));
  ...
  it('supports array API responses and the default status update error message', fakeAsync(() => {
    api.getVendorOrders.and.returnValue(of([order({ id: 'order-array' })] as any));
    fixture.detectChanges();
    tick(0);
