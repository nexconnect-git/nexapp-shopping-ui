...
export class DispatchBoardComponent implements OnInit, OnDestroy {
  ...
  activeOrders = computed(() => this.orders().filter(o => !['delivered', 'cancelled'].includes(o.status)));
  readyOrders = computed(() => this.orders().filter(o => o.status === 'ready'));
  inTransit = computed(() => this.orders().filter(o => ['picked_up', 'on_the_way'].includes(o.status)));
  pendingDispatch = computed(() => this.orders().filter(o => ['placed', 'confirmed', 'preparing'].includes(o.status)));
  onlinePartners = computed(() => this.partners().filter(p => p.status === 'active' || p.is_online || p.is_available));
  ...
  load(showLoading = true) {
    ...
    this.api.getAdminOrders({ page_size: 100, ordering: '-placed_at' }).subscribe({
      next: (res) => {
  ...
  lane(status: string): any[] {
    return this.orders().filter(o => o.status === status).slice(0, 8);
  }

  ageMinutes(order: any): number {
    const raw = order.placed_at || order.created_at;
    if (!raw) return 0;
    return Math.max(0, Math.round((Date.now() - new Date(raw).getTime()) / 60000));
  }

  slaClass(order: any): string {
    const age = this.ageMinutes(order);
    if (age >= 35 && !['picked_up', 'on_the_way'].includes(order.status)) return 'critical';
