...
export class PlatformSettingsComponent implements OnInit {
  ...
  paymentMethods = [
    ...
    },
    {
      key: 'razorpay_wallet',
      label: 'Wallets',
      description: 'Amazon Pay, Paytm, Mobikwik',
    },
    {
    ...
    },
    {
      ...
      description: 'Allow customers to pay at handoff',
    },
  ];
  ...
  load() {
    ...
    this.api.getPlatformSettings().subscribe({
      next: (settings) => {
        this.form = { ...settings };
        this.form.enabled_payment_methods = Array.isArray(
          this.form.enabled_payment_methods,
        )
          ? this.form.enabled_payment_methods
          : [];
        if (Array.isArray(this.form.cancellation_allowed_statuses)) {
          this.form.cancellation_allowed_statuses =
            this.form.cancellation_allowed_statuses.join(',');
        }
  ...
  save() {
    const payload = {
      ...this.form,
      enabled_payment_methods: Array.isArray(this.form.enabled_payment_methods)
        ? this.form.enabled_payment_methods
        : [],
      cancellation_allowed_statuses: String(
        this.form.cancellation_allowed_statuses || '',
      )
        .split(',')
        .map((s: string) => s.trim())
  ...
  isPaymentMethodEnabled(key: string) {
    return (
      Array.isArray(this.form.enabled_payment_methods) &&
      this.form.enabled_payment_methods.includes(key)
    );
  }

  togglePaymentMethod(key: string) {
    const current = Array.isArray(this.form.enabled_payment_methods)
      ? [...this.form.enabled_payment_methods]
      : [];
