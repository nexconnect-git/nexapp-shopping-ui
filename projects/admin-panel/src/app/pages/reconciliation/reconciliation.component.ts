...
export class ReconciliationComponent implements OnInit {
  ...
  load() {
    ...
    this.api
      .getAdminPayments({ page_size: 100, ordering: '-placed_at' })
      .subscribe({
        next: (res) => {
          this.payments.set(res.results || res || []);
          done();
        },
        error: () => {
          this.errors.update((e) => [...e, 'Payments could not be loaded.']);
          done();
        },
      });
    ...
    this.api.getAdminVendorPayouts({ page_size: 100 }).subscribe({
      next: (res) => {
        this.vendorPayouts.set(res.results || res || []);
        done();
      },
      error: () => {
        this.errors.update((e) => [
          ...e,
          'Vendor payouts could not be loaded.',
        ]);
        done();
      },
    });
    ...
    this.api.getAdminDeliveryPayouts({ page_size: 100 }).subscribe({
      next: (res) => {
        this.deliveryPayouts.set(res.results || res || []);
        done();
      },
      error: () => {
        this.errors.update((e) => [
          ...e,
          'Delivery payouts could not be loaded.',
        ]);
        done();
      },
    });
  ...
  get vendorPayable(): number {
    return this.vendorPayouts()
      .filter((p) =>
        ['pending_approval', 'approved', 'scheduled'].includes(p.status),
      )
      .reduce((sum, p) => sum + Number(p.net_payout || p.amount || 0), 0);
  }

  get deliveryPayable(): number {
    return this.deliveryPayouts()
  ...
  get exceptions(): any[] {
    return [
      ...this.payments()
        ...
        .map((p) => ({
          type: 'Payment',
          label: `Order #${p.order_number}`,
          status: 'Unverified Razorpay payment',
          route: `/orders/${p.id}`,
        })),
      ...this.vendorPayouts()
        .filter((p) => ['failed', 'declined'].includes(p.status))
        .map((p) => ({
          type: 'Vendor payout',
          label: p.vendor_name || p.id,
          status: p.status,
          route: '/payouts',
        })),
      ...this.deliveryPayouts()
        .filter((p) => ['failed', 'declined'].includes(p.status))
        .map((p) => ({
          type: 'Delivery payout',
          label: p.partner_name || p.id,
          status: p.status,
          route: '/payouts',
        })),
    ].slice(0, 12);
