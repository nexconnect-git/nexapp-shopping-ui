...
export class VendorOnboardingComponent {
    ...
    switch (step) {
      case 1:
        ...
        if (this.s1.password) {
          ...
          if (this.s1.password !== this.s1.confirm_password)
      ...
      case 4:
        ...
        if (
          this.s4.ifsc_code &&
          !/^[A-Z]{4}0[A-Z0-9]{6}$/i.test(this.s4.ifsc_code)
        )
  ...
  addHoliday() {
    ...
    this.s6.holidays.push({
      date: d,
      reason: this.s6.new_holiday_reason.trim(),
    });
  ...
  // ── Business addresses ────────────────────────────────────────────────────
  addBusinessAddress() {
    this.s3.business_addresses.push({
      line1: '',
      city: '',
      state: '',
      pincode: '',
